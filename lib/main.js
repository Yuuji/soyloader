    ...
    var buildSoyIntern = function(file) {
        ...
        if (options.plugins) {
            for (var key in options.plugins) {
                ...
                plugins += ' --pluginModules ' + plugin.namespace;
            }
        }
        
        exec('/usr/bin/java -classpath ' + jarFile + ' com.google.template.soy.SoyToJsSrcCompiler ' + plugins + ' --codeStyle ' + type + ' --outputPathFormat ' + tempName + ' ' + options.templateDir + file, function (error, stdout, stderr) {
            buildSoyInProgress--;
            setTimeout(buildSoyQueueProcess, 100);
            if (error !== null) {
